...
struct TourData nearestAddition(double **distances, int numOfCoords, int starting_coord) {
...
//    start = omp_get_wtime();;
    ...
    for(i = 0; i< numOfCoords; i++)
    {
        struct TourData tempTour = nearestAddition(distanceMatrix, numOfCoords, i);
...
//    time_taken = (end - start);
    ...
    printf("Nearest insertion for %d nodes completed\n", numOfCoords);
